        ...
        -> MKAnnotationView?
    {
        ...
        if let poiAnnotation = annotation as? POIAnnotation {
            ...
            case "restaurant", "food":
                annotationView?.image = UIImage(named: "PropertyRestaurant")?
                    .withRenderingMode(.alwaysOriginal)
            //                annotationView?.glyphText = nil
            case "cafe", "bakery":
                annotationView?.image = UIImage(named: "PropertyCafe")?
                    .withRenderingMode(.alwaysOriginal)
            //                annotationView?.glyphText = nil
            default:
        ...
        if let title = annotation.title ?? "", title.hasPrefix("도보") {
            ...
            let timeString = title.replacingOccurrences(of: "도보", with: "")
            ...
            let hostingController = UIHostingController(
                rootView: TimeAnnotation(timeText: timeString)
            )
            ...
            NSLayoutConstraint.activate([
                hostingController.view.centerXAnchor.constraint(
                    equalTo: annotationView.centerXAnchor
                ),
                hostingController.view.centerYAnchor.constraint(
                    equalTo: annotationView.centerYAnchor
                ),
            ])
